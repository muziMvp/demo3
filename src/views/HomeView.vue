  ...
  <div class="home">
    ...
    <div class="section">
      ...
      <div class="scroll-view">
        <div class="item" v-for="item in state.intheatersD" :key="item.id">
    ...
    <div class="section">
      ...
      <div class="scroll-view">
        <div class="item" v-for="item in state.comingSoonD" :key="item.id">
          <router-link :to="'/detail/'+item.id">
            <img :src="item.images.small" />
            <span
              class="title"
    ...
    <div class="section">
      ...
      <div class="scroll-view">
        <div class="item" v-for="item in state.top250D" :key="item.id">
          <router-link :to="'/detail/'+item.id">
            <img :src="item.images.small" />
            <span
              class="title"
...
<script setup>
import {ref,reactive,toRef,toRefs,getCurrentInstance,onBeforeMount,
  onMounted,
  onBeforeUnmount,
  provide} from 'vue';
...
import comingSoonData from '@/assets/json/comingSoonData'; 
import top250Data from '@/assets/json/top250Data'; 
...
const getTheater=()=> {
  // state.theatersD=theatersData.subjects
  // 处理数据 额外的增加一个 星星个数字段
  theatersData.subjects.forEach(v => {
    // 为V增加一个字段 纯数字无法循环
    // 必须弄成数组
  ...
  console.log(theatersData.subjects,'theatersData.subjects')
  state.intheatersD = theatersData.subjects
};
const getComingSoon=()=> {
  comingSoonData.subjects.forEach(v => {
    // 为V增加一个字段 纯数字无法循环
    // 必须弄成数组
    // v.startNum = Math.floor(v.rating.stars/10);
    v.startArr = []
  ...
  state.comingSoonD=comingSoonData.subjects
};
const getTop250=()=> {
  top250Data.subjects.forEach(v => {
    // 为V增加一个字段 纯数字无法循环
    // 必须弄成数组
    // v.startNum = Math.floor(v.rating.stars/10);
    v.startArr = []
  ...
  state.top250D=top250Data.subjects
};
const goToList = type => {
